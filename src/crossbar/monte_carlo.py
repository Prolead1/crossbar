...
def price_barrier_cv(
    bs: BSParams, bar: BarrierSpec, n_steps: int, n_paths: int, seed: int = 0
) -> np.ndarray:
    ...
    estimate.
    """
    X, Y = price_barrier(bs, bar, n_steps=n_steps, n_paths=n_paths, seed=seed)
...
def price_barrier_mc(
    ...
    """
    if control_variate:
        samples = price_barrier_cv(bs, bar, n_steps=n_steps, n_paths=n_paths, seed=seed)
    else:
        samples = price_barrier(bs, bar, n_steps=n_steps, n_paths=n_paths, seed=seed)[0]
